...
* **GPT Image 2** — no trigger word exists. The style comes from this game's own references: the
...
from .loras import LoraModel, resolve_lora
...
def build_prompt(
    ...
    if effective == Backend.HUGGING_FACE:
        return lora_prompt(resolve_lora(request.lora_name, request.game), request.prompt)
    return reference_prompt(request, descriptions or {})


def lora_prompt(model: LoraModel, subject: str) -> str:
...
def reference_prompt(request: ArtRequest, descriptions: dict[str, str]) -> str:
    """Style extracted from this game's own references, for a backend with no trigger word."""
    ...
    return (
        f"Concept art for {request.game}. Subject: {subject}. "
        f"Match only the visual language of the attached reference images for this game: "
