...
from concept_art_generator.art_models import ART_MODELS, BY_NAME
...
DRONE = BY_NAME["drone-bc"]


def hf_request(prompt, art_model="drone-bc"):
    return ArtRequest(art_model, prompt, Backend.HUGGING_FACE)
...
def test_gpt_prompt_honours_an_opaque_request():
    request = ArtRequest("drone-bc", "racer", Backend.GPT_IMAGE_2, transparent=False)
...
def test_build_prompt_refuses_an_uncatalogued_art_model():
    with pytest.raises(ValueError, match="Unknown art model"):
        build_prompt(hf_request("A drone", art_model="drone-mw"))
...
def seeded(tmp_path, hf) -> tuple[ConceptArtWorkflow, Capturing]:
    ...
    flow.add_reference("drone-bc", source, "White egg-shaped drone shell, lime thruster rings")
    return flow, gpt
...
def test_hugging_face_draft_and_final_send_the_identical_lora_prompt(tmp_path: Path):
    ...
    flow.create_final("drone-bc", job.id)
